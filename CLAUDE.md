...
| **Classe da emissora / raio de cobertura / cidades no alcance** (planos Anatel PBFM+PBOM, `stations.anatel_*`, `station_coverage_cities`, `backfill-anatel`) | [docs/features/anatel-station-class-coverage.md](docs/features/anatel-station-class-coverage.md) — o cruzamento é por **dial + geografia** (os planos não têm nome de emissora), então cada match guarda procedência (`anatel_match_tier`). **dois raios**: `anatel_coverage_km` = contorno protegido (normativo, usado pra identificar a emissora) e `anatel_reach_km` = contorno × 1,5 = **transbordo**, que é o raio que gera `station_coverage_cities` (fator herdado do E-radios pra os 2 sistemas baterem). **`anatel_coverage_km IS NULL` = indeterminado, NUNCA zero**: AM não tem raio por classe (norma define contorno em mV/m). Classe `C` de FM ≠ classe `C` de AM — desambigue por `band` |
...
| **Assertividade da plataforma** (card do `/management`, tabela `assertiveness_daily`, job a cada 6h) | [docs/features/assertiveness-metric.md](docs/features/assertiveness-metric.md) — janela é **sempre o último mês fechado** e ignora o filtro de período (mês em curso mente pra cima: manual chega depois). A métrica só enxerga miss **reportado**, então o card **nunca** mostra o % sem o volume ao lado. Fonte = Go; `scripts/sql/assertividade.sql` é a **verificação** — mexeu num, mexa no outro |
...
| Mapa ao Vivo (`/live-map`, admin + cliente — emissoras no mapa com **raio de cobertura** e cidades no alcance, tour por emissora + feed de veiculações em tempo real) | [docs/features/live-map.md](docs/features/live-map.md) — componente `CoverageMap.jsx`; o **`BrazilMap.jsx` NÃO foi substituído** (serve `/management` e o PNG do pós-venda — mudar as props dele quebra o envio do pós-venda). Raio e cidades só são legíveis em **escalas diferentes**: panorama enquadra a pegada da campanha, foco enquadra uma emissora. Nada de WebGL — o botão "Baixar" usa `html2canvas`, que não fotografa canvas |
