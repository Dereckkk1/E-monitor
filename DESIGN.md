...
5. **Proibição de Componentes Nativos Imperativos:** Nunca utilize `window.alert`, `window.confirm` ou `window.prompt`. A plataforma da Signalads usa uma experiência Web App fluida, de modo que confirmações, sucessos ou exclusões devem acontecer através de interações ou Modais desenhadas no ecossistema.
   ...
   **Implementação no Radiocheck — `ConfirmProvider`:**
   - O `ConfirmProvider` em [`frontend/src/components/ConfirmModal.jsx`](frontend/src/components/ConfirmModal.jsx) é montado uma vez na raiz da aplicação e intercepta tanto `window.confirm` quanto `window.alert`, substituindo-os por modais customizadas. Não há provider/hook adicional — você usa as APIs nativas de `window` normalmente e a interceptação acontece de forma transparente.
   - **Confirmações** (`window.confirm`): retornam `Promise<boolean>`. Sempre `await` e desramifique:
     ```js
     const ok = await window.confirm('Excluir este material? Não pode desfazer.')
     if (!ok) return
     ```
   - **Alertas/avisos** (`window.alert`): retornam `Promise<void>`. Pode ou não dar `await`:
     ```js
     window.alert('Material já tem detecções e não pode ser excluído.')
     ```
   - **Limitação importante (não usar para o que a modal não aguenta):**
     - O modal exibe **apenas uma string de mensagem**. Não suporta JSX, links clicáveis, listas formatadas, ícones customizados nem múltiplos botões além do par padrão (Confirmar/Cancelar) ou OK único.
     - O `confirm` tem botões fixos em pt-BR ("Cancelar" / "Confirmar") com variante danger. Não é parametrizável.
     - O `alert` tem um único botão "OK". Não suporta tipos visuais (success/warning/error) — todos saem como aviso neutro.
     - **Não jogue stack traces, JSON cru ou textos com mais de ~280 caracteres.** A modal é compacta e textos longos quebram a hierarquia visual. Para detalhes longos, use uma página dedicada ou um toast com link "ver detalhes".
     - Não use para fluxos que precisam de input do usuário (`window.prompt` não é interceptado e não deve ser usado).
   - **Quando precisar de mais que uma string:** crie uma modal dedicada (ver §4.3) com `createPortal` em vez de tentar abusar do `alert/confirm`. O `ConfirmProvider` é só pra confirmações binárias e avisos curtos.
   - Acessibilidade já incluída: `role="dialog"`, `aria-modal="true"`, foco automático no botão padrão, `Esc` cancela/fecha, `Enter` confirma.
