-- 0029_daily_summary_exclude_audit_rejected.up.sql
-- Exclui detections com evidence_status = 'audit_rejected' do agregado
-- daily_play_summary.
--
-- Contexto: a feature §9.9 (migration 0028, doc docs/architecture/evidence-audit.md)
-- estampa evidence_status='audit_rejected' em detections cujo clipe salvo não
-- bate com o master atribuído — falsos positivos forenses. Os list endpoints
-- em workers/internal/catalog/detections.go já filtram esse status, mas a view
-- daily_play_summary ficou pra trás, contando audit_rejected nos badges
-- in_slot/out_slot/out_date/orphan. Resultado: a grid /detections mostrava
-- "N veiculações" mas o DayDetailModal exibia M < N, com a diferença sendo
-- exatamente as rejeitadas pelo audit. CoverageSummary também inflava
-- denominador no cálculo de cobertura.
--
-- Fix: adicionar `AND d.evidence_status <> 'audit_rejected'` ao CTE `actual`,
-- mesmo padrão usado por 0020 quando ignored_at foi introduzido. Schema da
-- view permanece idêntico — CREATE OR REPLACE substitui a definição sem
-- precisar dropar dependências.
