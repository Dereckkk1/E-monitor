...
- É a base que **fatura**. `/campaigns` sempre usou ela (`total_insertions`, `total_audience`), e no `/insights` "Investido (executado)" (= `in_slot`) + "Bonificação" (= `bonus`) cobrem exatamente esse conjunto. Desde 2026-08-17 o `/campaigns` parte a base do mesmo jeito no dinheiro: `total_invested = unit × in_slot` e `total_bonus_value = unit × bonus` (o bônus é entrega gratuita e saiu do investido). **Impactos e inserções continuam somando `in_slot + bonus` nas duas telas** — a tocada aconteceu e a audiência ouviu; só o dinheiro se parte. A identidade está travada por teste: `TestInsights_FinancialBase_MatchesCampaigns`, que assere as duas parcelas financeiras e `insights.Impactos == campaigns.TotalAudience`.
