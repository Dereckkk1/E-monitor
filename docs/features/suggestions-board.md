...
- **`suggestion_attachments`** — imagens no S3 (`suggestion_id` + `comment_id` nullable); `URL` presigned preenchido no handler.
...
| `GET` | `/suggestions/attachments/{aid}/url` | autor/dev | presigned GET 5min |
...
- `frontend/src/pages/suggestions/` — componentes: `CentralDeComando`, `MinhasSugestoes`, `SuggestionDetail` (drawer), `SuggestionCreateModal`, `SuggestionThread`, `SuggestionPills`, `AttachmentLightbox`, `ClipboardPasteZone` (colar print com Ctrl+V), `EmptyState` (ghost preview), `Skeletons` (shimmer), `icons.jsx` (SVG inline — sem lib nova, ver CLAUDE.md §5), `constants.js`, `utils.js`.
