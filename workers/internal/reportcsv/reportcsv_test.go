...
func TestWriteDetailed_PrecoSoNaLinhaDentroDaFaixa(t *testing.T) {
	...
	_, lines := writeDetailed(t, []catalog.DetectionEnriched{
		...
		detFixture(1, "A", "SC", "M", "bonus", &price),
		detFixture(1, "A", "SC", "M", "orphan", &price),
		detFixture(1, "A", "SC", "M", "in_slot", nil),
	})

	require.Contains(t, lines[1], "Dentro da faixa;812;R$ 53,03")
	// Fora da faixa / fora da data / bonificação não faturam: R$ 0,00 mesmo com
	// unit_value cadastrado. Sob a categorização por cota a out_slot deixou de
	// valer qualquer coisa, e o excedente da meta do dia vira `bonus` — nenhum
	// dos dois entra na coluna de preço.
	require.Contains(t, lines[2], "Fora da faixa;812;R$ 0,00")
	require.Contains(t, lines[3], "Fora da data;812;R$ 0,00")
	require.Contains(t, lines[4], "Bonificação;812;R$ 0,00")
	// 'orphan' é o nome legado de 'bonus' (migration 0064): mesma linha, mesmo
	// rótulo, mesmo preço zero.
	require.Contains(t, lines[5], "Bonificação;812;R$ 0,00")
	// in_slot sem pricing cadastrado também é R$ 0,00.
	require.Contains(t, lines[6], "Dentro da faixa;812;R$ 0,00")
	require.NotContains(t, strings.Join(lines, "\n"), "orphan")
}

// Rótulos do vocabulário PT-BR do DayDetailModal, não o enum cru do banco — e
// 'bonus' e o sinônimo legado 'orphan' compartilham o MESMO rótulo, porque na
// janela de deploy o binário antigo ainda grava 'orphan' e o CSV é o arquivo
// que o cliente abre.
func TestWriteDetailed_TraduzCategoria(t *testing.T) {
	out, _ := writeDetailed(t, []catalog.DetectionEnriched{
		detFixture(1, "Radio X", "SC", "Spot 30s", "bonus", nil),
		detFixture(2, "Radio Y", "SC", "Spot 30s", "in_slot", nil),
		detFixture(3, "Radio Z", "SC", "Spot 30s", "orphan", nil),
	})
	...
	// O rótulo velho não pode reaparecer: o cabeçalho do consolidado virou
	// "Bonificação" no mesmo passo.
	require.NotContains(t, out, "Bônus")
}
