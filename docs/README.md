...
| [sincronizar-campanhas-com-o-hub.md](operations/sincronizar-campanhas-com-o-hub.md) | **Operacional.** Como o vinculo por codigo funciona, como esta em producao desde 2026-09-22, o que fazer com as campanhas que ja existiam (agrupar move a proposta; a auto-criada fica vazia), e o diagnostico de "a campanha nao aparece no hub" |
| [codigo-da-campanha.md](features/codigo-da-campanha.md) | O vínculo com o E-Hub pelo **código da campanha** (`EH-7K4M2X`): campo obrigatório no wizard, barreira de 422 no `POST`/`PUT`, `GET /v1/internal/hub-codes/{code}` para a tela conferir sem expor a chave, e job de 15min que reemite quem o hub não confirmou. **Implementado e não publicado** |
...
| [incident-2026-09-01-cpu-saturation-vm-resize.md](incidents/incident-2026-09-01-cpu-saturation-vm-resize.md) | 2026-09-01 | VM saturou a 268 emissoras (`id 0%`, PSI 76%, p99 da janela em 2,39 s contra cadência de 2 s). Causa: custo de matching cresce com **emissoras × tamanho do índice**, e o modelo de julho só via emissoras — início de mês ativou um bloco de campanhas e inchou o índice. Migração `c3-highcpu-8` → `c2d-standard-16` (C3 não tem shape de 16 vCPU; C4 bloqueado por exigir Hyperdisk). p99 → 34,4 ms. Achados colaterais: CUD já existia (flex 28%, não 37%), `alertmanager` fora do ar, 24/25 serviços sem política de restart, VM órfã em Montreal |
| [incident-2026-09-02-detections-station-hidden-by-catalog-page.md](incidents/incident-2026-09-02-detections-station-hidden-by-catalog-page.md) | 2026-09-02 | Campanha 191 (UNIUBE) mostrava 3 de 4 emissoras na grade de `/detections` — e o rodapé contava 4. `GET /stations?limit=2000` é a 1ª página de 7,5 mil emissoras ordenada por (monitoring_status, pmm DESC NULLS LAST, name): ao terminar a campanha o supervisor pausa a emissora, e a única com `pmm NULL` caiu pra posição ~2.726. A grade descartava a linha não resolvida em silêncio (`if (!station) return null`), levando junto 258 veiculações — no CSV/PDF também. Fix: resolver por `?ids=` (conjunto exato, fatiado em 500) + placeholder em vez de sumiço. `/campaigns` segue pendente |
